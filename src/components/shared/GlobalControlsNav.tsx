'use client';

import React from 'react';
import Link from 'next/link';
import { useAppContext } from '@/lib/context/AppContext';

export default function GlobalControlsNav() {
  const { settings } = useAppContext();

  return (
    <div className="fixed top-4 left-4 z-50">
      <Link
        href="/admin/controls"
        className="inline-flex items-center px-4 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200"
      >
        <svg
          className="w-5 h-5 mr-2 text-gray-600 dark:text-gray-300"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth={2}
            d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"
          />
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth={2}
            d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"
          />
        </svg>
        <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
          Global Controls
        </span>
        
        {/* Status indicators */}
        <div className="ml-2 flex space-x-1">
          {settings.isDarkMode && (
            <div className="w-2 h-2 bg-blue-500 rounded-full" title="Dark Mode Active" />
          )}
          {!settings.useTestData && (
            <div className="w-2 h-2 bg-green-500 rounded-full" title="Live Data Active" />
          )}
          {!settings.isAutoPlayEnabled && (
            <div className="w-2 h-2 bg-yellow-500 rounded-full" title="Auto Play Disabled" />
          )}
        </div>
      </Link>
    </div>
  );
} 